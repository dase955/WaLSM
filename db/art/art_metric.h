...
// 将NVMWriteMetric和SSDWriteMetric加起来才是总的Write Data Size
...
struct NVMWriteMetric {
    ...
    void printMetric() {
        if (WriteSsdDataBytes - LastPrintedBytes > 512 * 1024 * 1024) {
...
struct FlushMetric {
    ...
    void printMetric() {
        if (FlushSsdDataBytes - LastPrintedBytes > 256 * 1024 * 1024) {
...
struct ReadMetric {
    ...
    void updateMetric(uint64_t offset_start, uint64_t offset_end) {
        ...
        if (EVALUATE_METRIC) {
            ReadSsdBlocksCnt += (block_end - block_start + 1);
            printMetric();
        }
    }
};
