...
class KeyGenerator {
 public:
  KeyGenerator(int record_count, int64_t sample_range)
...
class YCSBZipfianGenerator : public KeyGenerator{
 public:
  YCSBZipfianGenerator(int record_count, int64_t sample_range, double theta, double zetan)
  ...
  uint64_t Next() {
    ...
    auto value = (int64_t) ((sample_range_) * std::pow(eta * u - eta + 1, alpha));
    return fnvhash64(value) % sample_range_;
  }
...
class CustomZipfianGenerator : public KeyGenerator {
 public:
  CustomZipfianGenerator(int record_count, int64_t sample_range, double alpha)
  ...
  void Prepare() override {
    double c = 0.0;
    for (int64_t i = 1; i <= sample_range_; i++) {
    ...
    std::unordered_map<int64_t, int64_t> modified;
 ...
 private:
  static void ShuffleSamples(
    ...
    std::random_device rd;
    std::default_random_engine rng = std::default_random_engine{rd()};
    std::uniform_int_distribution<int64_t> dist(0, sample_range - 1);

    int64_t left[2] = {400000000, 800000000};
    int64_t interval = 50000000;
    int64_t mod = interval * 2;

    std::random_device shuffle_rd;
    std::vector<int64_t> shuffled(mod);
    std::iota(std::begin(shuffled), std::end(shuffled), 0);
    std::shuffle(shuffled.begin(), shuffled.end(), shuffle_rd);

    auto check_func = [&](int64_t val) {
      for (auto l : left) {
  ...
  static void CheckFreq(std::vector<int64_t>& samples) {
    std::unordered_map<int64_t, int> freqs;
...
int main(int argc, char* argv[]) {
  ...
  std::string db_path = "/tmp/db_nvm_l0";
