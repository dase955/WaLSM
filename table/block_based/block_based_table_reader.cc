...
bool BlockBasedTable::FullFilterKeyMayMatch(
  ...
  if (rep_->whole_key_filtering) {
    size_t ts_sz =
        rep_->internal_comparator.user_comparator()->timestamp_size();
    Slice user_key_without_ts = StripTimestampFromUserKey(user_key, ts_sz);
    // add filter_cache argument here
    may_match =
        filter->KeyMayMatch(filter_cache,
                            user_key_without_ts, prefix_extractor, kNotValid,
                            no_io, const_ikey_ptr, get_context, lookup_context);
  } else if (!read_options.total_order_seek && prefix_extractor &&
             rep_->table_properties->prefix_extractor_name.compare(
                 prefix_extractor->Name()) == 0 &&
             ...
             !filter->PrefixMayMatch(prefix_extractor->Transform(user_key),
                                     ...
                                     const_ikey_ptr, get_context,
                                     lookup_context)) {
    // WaLSM+ dont use prefix filter, so this branch will not reach
    may_match = false;
  }
  if (may_match) {
    RecordTick(rep_->ioptions.statistics, BLOOM_FILTER_FULL_POSITIVE);
...
#ifdef ART_PLUS
Status BlockBasedTable::Get(FilterCacheClient& filter_cache,
  ...
  const bool may_match =
      FullFilterKeyMayMatch(filter_cache, 
                            read_options, filter, key, no_io, prefix_extractor,
                            get_context, &lookup_context);
  TEST_SYNC_POINT("BlockBasedTable::Get:AfterFilterMatch");
  if (!may_match) {
    RecordTick(rep_->ioptions.statistics, BLOOM_FILTER_USEFUL);
